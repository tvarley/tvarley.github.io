...
[Triangular number](https://grokipedia.com/page/Triangular_number)s are generated by the formula $T_n = \frac{n(n+1)}{2}$. They represent the sum of the first n natural numbers and form a quadratic sequence.

The number of [divisor function](https://grokipedia.com/page/Divisor_function) $d(n)$ counts how many positive divisors n has. For a number with [prime factorization](https://grokipedia.com/page/Integer_factorization) $n = p_1^{a_1} \times p_2^{a_2} \times \dots \times p_k^{a_k}$, the number of divisors is $d(n) = (a_1 + 1)(a_2 + 1) \dots (a_k + 1)$.

[Highly composite number](https://grokipedia.com/page/Highly_composite_number)s (numbers with many divisors) often result from having many small prime factors. Triangle numbers can be highly composite because they may share factors with many other numbers.
